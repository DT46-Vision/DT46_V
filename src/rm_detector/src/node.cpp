...
#include "rm_interfaces/msg/light_info.hpp"
#include "rm_interfaces/msg/lights_msg.hpp"
...
namespace DT46_VISION {
    ...
    public:
        ArmorDetectorNode() : Node("rm_detector") {
            ...
            publisher_lights_     = this->create_publisher<rm_interfaces::msg::LightsMsg>("/detector/lights", 10);
    ...
    private:
        ...
        void processing_loop() {
            ...
            while (rclcpp::ok() && running_.load()) {
                ...
                rm_interfaces::msg::LightsMsg lights_msg;
                lights_msg.header = armors_msg.header;
                ...
                if (!detection_error && !armors.empty()) {
                    ...
                    for (const auto& armor : armors) {
                        ...
                        armor_info.light1_up_x = armor.light1_up.x;
                        armor_info.light1_up_y = armor.light1_up.y;
                        armor_info.light1_down_x = armor.light1_down.x;
                        armor_info.light1_down_y = armor.light1_down.y;
                        armor_info.light2_up_x = armor.light2_up.x;
                        armor_info.light2_up_y = armor.light2_up.y;
                        armor_info.light2_down_x = armor.light2_down.x;
                        armor_info.light2_down_y = armor.light2_down.y;
                ...
                if (!detection_error) {
                    for (const auto& light : detector_->lights) {
                        rm_interfaces::msg::LightInfo li;
                        li.color = light.color;
                        li.cx = static_cast<float>(light.cx);
                        li.cy = static_cast<float>(light.cy);
                        li.up_x = light.up.x;
                        li.up_y = light.up.y;
                        li.down_x = light.down.x;
                        li.down_y = light.down.y;
                        lights_msg.lights.push_back(li);
                    }
                }

                publisher_armors_->publish(armors_msg);
                publisher_lights_->publish(lights_msg);
    ...
    private:
        ...
        rclcpp::Publisher<rm_interfaces::msg::LightsMsg>::SharedPtr  publisher_lights_;
