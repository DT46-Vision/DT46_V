...
import cv2
import numpy as np
from rm_opencv_aim.rm_tracker.rm_tracker.test.filter import ArmorFilter  # 你若保存为 filter.py，用这一行
...
class Tracker:
    ...
    def track_armor(self, msg):
        ...
        # tracking_robot_center = self.estimate_robot_center(tracking_armor_filtered)
        # tracking_robot = self.estimate_robot(tracking_robot_center)
